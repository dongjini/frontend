...
document.querySelector("button").addEventListener("click", () => {
  ...
  const url = `http://www.kobis.or.kr/kobisopenapi/webservice/rest/boxoffice/searchDailyBoxOfficeList.json?key=6680fd16764e73706ac52fe236136c15&targetDt=${targetDt}`;

  fetch(url)
    .then((res) => res.json())
    .then((date) => {
      console.log(date.boxOfficeResult);
      console.log(date.boxOfficeResult.dailyBoxOfficeList);

      const boxList = date.boxOfficeResult.dailyBoxOfficeList;
...
const movieInfo = (movieCd) => {
  console.log(movieCd);
  const url = `http://www.kobis.or.kr/kobisopenapi/webservice/rest/movie/searchMovieInfo.json?key=6680fd16764e73706ac52fe236136c15&movieCd=${movieCd}`;
