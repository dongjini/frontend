...
const others = ["과일", 1, 3.14, new Date(), true, {}]; // 서로 다른걸 담아도 상관없음
...
console.log("배열 분활 " + fruits.slice(1, 3));
console.log("배열 정렬 " + fruits.sort()); //  문자 정렬
// 숫자 정렬 X
const array2 = [22, 35, 1, 3, 7, 88, 92];
console.log("배열 정렬 " + array2.sort()); // 숫자는 정렬 안해줌
// 오름차순 정렬
console.log("배열 오름차순 정렬 : " + array2.sort((a, b) => a - b));
console.log("배열 내림차순 정렬 : " + array2.sort((a, b) => b - a));
console.log("베열 거꾸로 출력 : " + fruits.reverse());
...
let fruits2 = fruits; // 얕은복사 : 동일한 주소값( 같은 공간을 가르켜 변화에 서로 영향 받음)
// ... : spread 연산자
...
const array4 = new Array(5).fill(1); // 5개의 빈 방에 1로 채움.
...
let toArr = [varArr1, varArr2]; // 주소복사
let toArr2 = [...varArr1, ...varArr2]; // 값복사
